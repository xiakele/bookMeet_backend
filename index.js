...
const port = 3030
const startScrapers = require(path.join(__dirname, '/startScrapers'))
const search = require(path.join(__dirname, '/scrapers/search'))
...
async function start () {
  ...
  app.get(['/douban', '/dangdang', '/chaoxing', '/booksChina'], async (req, res) => {
    ...
    if (req.query.c) {
      if (req.path === '/douban' && subCats.indexOf(req.query.c) !== -1) {
        data = await readJSON(path.join(__dirname, `/results/douban_${req.query.c}.json`))
      } else {
        return res.status(400).json({ category: req.path.slice(1), time: -1, data: [] })
      }
    } else {
      data = await readJSON(path.join(__dirname, `/results/${req.path.slice(1)}.json`))
    }
    if (req.query.n) {
  ...
  app.get('/getTags', async (req, res) => {
    ...
    try {
      try {
        const localData = JSON.parse(await fs.readFile(path.join(__dirname, `/results/tags/${req.query.id}.json`)))
