...
module.exports = async function start ({ page }) {
  let books = []
  const statCode = (await page.goto('https://book.douban.com/latest?subcat=%E6%96%87%E5%AD%A6')).status()
  if (statCode === 403) {
  ...
  for (let i = 1; i <= pageCnt; i++) {
    await page.goto(`https://book.douban.com/latest?subcat=%E6%96%87%E5%AD%A6p=${i}`, { waitUntil: 'domcontentloaded' })
    books = books.concat(await getBooks(page))
  }
  const updateTime = new Date().getTime()
  books = { category: 'douban_literature', time: updateTime, data: books }
  await fs.writeFile(path.join(__dirname, '/../results/douban_literature.json'), JSON.stringify(books))
  console.log(`douban_literature updated successfully at ${new Date()}`)
}
